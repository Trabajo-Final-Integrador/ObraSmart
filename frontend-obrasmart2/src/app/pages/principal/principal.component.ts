...
export class PrincipalComponent implements AfterViewInit {
      ...
      next: (data: EquipoUbicacion[]) => {
        ...
        this.equipos.forEach((eq) => {
          if (eq.lat !== 0 && eq.lon !== 0 && eq.lat && eq.lon) {
            ...
            console.warn(`Coordenadas inválidas para ${eq.nombre}: ${eq.lat}, ${eq.lon}`);
          }
        });
      },
      error: (err) => {
  ...
  private getIconByEstado(estado: string): L.DivIcon {
    ...
    if (estadoNormalizado.includes('OPERATIVO')) return iconOperativo;
    if (estadoNormalizado.includes('MANTENIMIENTO')) return iconMantenimiento;
    if (estadoNormalizado.includes('FUERA') || estadoNormalizado.includes('REPARACION')) return iconFueraServicio;
